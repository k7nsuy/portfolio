...
function serialize(item: Item): IRow.Payload {
  ...
  const startedAt = DateTime.fromFormat(item.startedAt, DATE_FORMAT.LESS_MONTH_YEAR).toFormat(
    DATE_FORMAT.LESS_MONTH_YEAR,
  );
  const title = (() => {
    if (item.endedAt) {
      const endedAt = DateTime.fromFormat(item.endedAt, DATE_FORMAT.LESS_MONTH_YEAR).toFormat(
        DATE_FORMAT.LESS_MONTH_YEAR,
      );
