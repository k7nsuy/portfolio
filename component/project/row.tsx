...
function serialize(payload: IProject.Item): IRow.Payload {
  ...
  const startedAt = DateTime.fromFormat(payload.startedAt, DATE_FORMAT.MONTH_YEAR).toFormat(
    DATE_FORMAT.MONTH_YEAR,
  );
  const title = (() => {
    if (payload.endedAt) {
      const endedAt = DateTime.fromFormat(payload.endedAt, DATE_FORMAT.MONTH_YEAR).toFormat(
        DATE_FORMAT.MONTH_YEAR,
      );
