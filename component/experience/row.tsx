...
function createWorkingPeriod(startedAtString: string, endedAtString?: string) {
  ...
  const startedAt = DateTime.fromFormat(startedAtString, DATE_FORMAT.LESS_MONTH_YEAR);

  const { periodTitle, endedAt, isWorking } = (() => {
    if (!endedAtString) {
      return {
        periodTitle: `${startedAt.toFormat(DATE_FORMAT.LESS_MONTH_YEAR)} ~`,
        isWorking: true,
        endedAt: undefined,
      };
    }

    const _endedAt = DateTime.fromFormat(endedAtString, DATE_FORMAT.LESS_MONTH_YEAR);
    return {
      periodTitle: `${startedAt.toFormat(DATE_FORMAT.LESS_MONTH_YEAR)} ~ ${_endedAt.toFormat(
        DATE_FORMAT.LESS_MONTH_YEAR,
      )}`,
      endedAt: _endedAt,
      isWorking: false,
    };
