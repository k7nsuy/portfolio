...
function getFormattingExperienceTotalDuration(payload: IExperience.Payload) {
    ...
    .map((item) => {
      return {
        endedAt: item.endedAt
          ? DateTime.fromFormat(item.endedAt, Util.LUXON_DATE_FORMAT.LESS_MONTH_YEAR)
          : DateTime.local(),
        startedAt: DateTime.fromFormat(item.startedAt, Util.LUXON_DATE_FORMAT.LESS_MONTH_YEAR),
      };
    })
  ...
  return totalExperience.toFormat(`${Util.LUXON_DATE_FORMAT.DURATION_YEARS_MONTHS}`);
}
